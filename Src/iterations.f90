...
contains
  ...
  subroutine iteration_g0(Nlist,Nequi,ALLEquiList,omega,tau_zero,tau_g)
    implicit none

    integer(kind=4),intent(in) :: Nlist,Nequi(Nlist),ALLEquiList(Nsymm_rot,nptk)
    real(kind=8),intent(in) :: omega(nptk,Nbands),tau_zero(Nbands,Nlist)
    real(kind=8),intent(out) :: tau_g(Nbands,nptk)
    ...
    do ll=1,Nlist
       do ii=1,Nbands
          do kk=1,Nequi(ll)
             tau_g(ii,ALLEquiList(kk,ll))=tau_zero(ii,ll)
          end do
       end do
    end do
  end subroutine iteration_g0
