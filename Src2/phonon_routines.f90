...
contains
  ...
  subroutine eigenDM(omega,eigenvect,velocity,velocity_g)
    implicit none

    include "mpif.h"

    real(kind=8),intent(out) :: omega(nptk,nbands),velocity(nptk,nbands,3),velocity_g(nptk,nbands,nbands,3)
    complex(kind=8),intent(out) :: eigenvect(nptk,Nbands,Nbands)

    real(kind=8),allocatable :: omega_reduce(:,:),velocity_reduce(:,:,:),velocity_reduce_g(:,:,:,:)
    ...
    allocate(omega_reduce(nptk,nbands),velocity_reduce(nptk,nbands,3),&
         eigenvect_reduce(nptk,Nbands,Nbands),velocity_reduce_g(nptk,nbands,nbands,3))
    omega_reduce=0.
    velocity_reduce=0.
    velocity_reduce_g=0. 
    eigenvect_reduce=0.
    ...
    if(espresso) then
       ...
       call phonon_phonopy(kspace(ii:jj,:),omega_reduce(ii:jj,:),&
            velocity_reduce(ii:jj,:,:),eigenvect_reduce(ii:jj,:,:),velocity_reduce_g(ii:jj,:,:,:))
    end if
    ...
    call MPI_ALLREDUCE(velocity_reduce_g,velocity_g,nptk*nbands*nbands*3,&
          MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,kk)
    call MPI_ALLREDUCE(eigenvect_reduce,eigenvect,nptk*nbands*nbands,&
         MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,kk)
    deallocate(omega_reduce,velocity_reduce,eigenvect_reduce,velocity_reduce_g)
  ...
  subroutine phonon_phonopy(kpoints,omegas,velocities,eigenvect,velocities_g)
    implicit none

    real(kind=8),intent(in) :: kpoints(:,:)
    real(kind=8),intent(out) ::omegas(:,:),velocities(:,:,:),velocities_g(:,:,:,:)
