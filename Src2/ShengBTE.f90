...
program ShengBTE
  ...
  integer(kind=4) :: i,j,ii,jj,kk,ll,mm,i1,i2
  integer(kind=4) :: Tcounter
  real(kind=8),allocatable :: energy(:,:),q0(:,:),q0_reduced(:,:),velocity(:,:,:),velocity_z(:,:),velocity_g(:,:,:,:)
  ...
  real(kind=8),allocatable :: tau_zero(:,:),tau(:,:),tau_b(:,:),tau2(:,:),tau_b2(:,:),tau_g(:,:)
  ...
  !Variables related to glass-like conductivity
  ...
  allocate(velocity_g(nptk,nbands,nbands,3)) 
  allocate(tau_g(nbands,nptk))
  ...
  call eigenDM(energy,eigenvect,velocity,velocity_g)
...
!     end do
  ...
  open(733,file="BTE.Kappa_Glass_like_RTA")
  if(convergence) then
  ...
  do Tcounter=1,CEILING((T_max-T_min)/T_step)+1
     ...
     ! WuYu
     call iteration_g0(Nlist,Nequi,ALLEquiList,energy,tau_zero,tau_g)
     ...
     if(myid.eq.0) then
        ...
        open(826,file="BTE.kappa_glass_resolved2",status="replace")
        ...
        do ll=1,nptk
           do i1=1,nbands
           call symmetrize_tensor(ThConductivityMode_g2(ll,i1,:,:))
        end do
        end do
        ...
        do i1=1,nbands
           do ll = 1,Nlist
              write(826,"(2E20.10,9E14.5)") energy(list(ll),i1),ThConductivityMode_g2(ll,i1,:,:)
           end do
       end do
        flush(826)
        ...
        close(2002)
        close(2003)
        open(496,file="BTE.kappa_particle_resolved",status="replace")
        do i1=1,nbands
                        do ll = 1,Nlist
                write(496,"(2E20.10,9E14.5)") energy(list(ll),i1),ThConductivityMode(ll,i1,:,:)
                        end do
                end do
        flush(496)
